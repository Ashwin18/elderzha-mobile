...
class _CheckInScreenState extends State<CheckInScreen> {
  ...
  Future<void> _loadMaster() async {
    ...
    setState(() {
      if (res != null && res['data'] != null) {
        ...
        _moodOpts =
            _parseOpts(_firstList(d, ['moods', 'mood', 'feelings'])) ??
        ...
        _actOpts =
            _parseOpts(
              _firstList(d, ['activities', 'activity', 'daily_activities']),
            ) ??
  ...
  Future<void> _submit() async {
    if (_mood.isEmpty || _weather.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Please complete the required check-in questions',
            style: poppins(13),
          ),
  ...
  Widget _iconGrid(
    ...
    const columns = 3;
    final rows = <Widget>[];
    for (int i = 0; i < opts.length; i += columns) {
      final slice = opts.sublist(i, (i + columns).clamp(0, opts.length));
      rows.add(
        Padding(
          ...
          child: Row(
            children: [
              ...
              // fill remaining cols
              ...List.generate(
                (columns - slice.length).clamp(0, columns),
      ...
      children: [
        Row(
          children: [
            ...
            Text(
              'Sleep time',
              style: poppins(12, w: FontWeight.w800, c: C.ink),
            ),
            const Spacer(),
            GestureDetector(
